...
def avg_corr_roll(S):
    ...
    C_uncertainty = np.zeros(Lx, dtype=S.dtype)
    N = Lx * Ly * 3
    ...
    for r in range(Lx):
        ...
        C_uncertainty[r] += np.einsum('ijij,ijij->', Sx, Sx.conj())

        C[r] += np.einsum('ijij->', Sy)
        C_uncertainty[r] += np.einsum('ijij,ijij->', Sy, Sy.conj())

        C[r] += np.einsum('ijij->', S_x_minus_y)
        C_uncertainty[r] += np.einsum('ijij,ijij->', S_x_minus_y, S_x_minus_y.conj())

    C = C / N
    C_uncertainty = np.sqrt(C_uncertainty / N - np.abs(C) ** 2)
...
data_file = "data/data_set_10"
correlator_type = "sz" # "psi"
...
correlation_all, correlation_uncertainty = avg_corr_roll(correlation_ij)

print(correlation_all)
print(correlation_uncertainty)
...
with shelve.open(data_file + "/results") as db:
    if correlator_type == "sz":
        db["correlation_sz"] = correlation_all
        db["correlation_sz_uncertainty"] = correlation_uncertainty
    if correlator_type == "psi":
        db["correlation_psi"] = correlation_all
        db["correlation_psi_uncertainty"] = correlation_uncertainty
